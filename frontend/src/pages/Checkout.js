...
function CheckoutPage() {
    ...
    const handleCheckout = async (event) => {
        ...
        if (data.url) {
            setAuthUrl(data.url);
            Cookies.set('stripeUrl', data.url, { expires: 1/720 }); // Set cookie to expire in 2 minutes
            console.log('Stripe URL set in cookie:', data.url);
        }
    };
    ...
    return (
        <div>
            ...
            {authUrl && <CriiptoAuthWrapper redirectUrl="https://distinctly-handy-eel.ngrok-free.app/callback" />}
            <Footer/>
        </div>
    );
}
